...
public class AlienHorde
{
	...
	public AlienHorde(int size)
	{
		...
		for (int i=0; i<size; i++) {
			aliens.add(new Alien(0,0,30,30,2));
		}
	}
	...
	public void moveEmAll()
	{
		for (int i=0; i<aliens.size(); i++) {
			...
			if (i==0 || aliens.get(i-1).getY()!=a.getY() || a.getX()+aliens.get(i-1).getWidth()+50 < aliens.get(i-1).getX() || a.getX() > aliens.get(i-1).getX() + aliens.get(i-1).getWidth() + 50) {
				if (a.getSpeed()>0 && a.getX() >= 800-a.getWidth()-a.getSpeed() || a.getSpeed()<0 && a.getX()<=-a.getSpeed()) {
